...
var stick = function(name, deps, next) {
	...
	// If the stick already, we add it as a first dependency
	var originalStick = sticks[name];
	if (originalStick) {
		if (deps === null) deps = [];
		deps.unshift(originalStick);
	}
	...
	var callback = function(params, func) {
		executeStick(stick, params, func);
	};
	callback.stick = stick;
	return callback;
};
...
var executeDependencies = function(deps, req, res, param, func) {
	...
	for (var i=0;i<deps.length;i++) {
		...
		if (typeof deps[i] == "object")
			stick = deps[i];
		else if (typeof deps[i] == "function")
			stick = deps[i].stick;
		else
			stick = sticks[deps[i]];
...
module.exports.execute = executeStick;
